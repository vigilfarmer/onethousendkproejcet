...
#twitch_miner = TwitchChannelPointsMiner("Smallbraincollector")
...
twitch_miner = TwitchChannelPointsMiner(
    ...
    logger_settings=LoggerSettings(
        ...
        discord=Discord(
            ...
            events=[Events.STREAMER_ONLINE, Events.STREAMER_OFFLINE, Events.CHAT_MENTION, Events.GAIN_FOR_CLAIM],                                  # Only these events will be sent to the chat
        ),
    ),
    streamer_settings=StreamerSettings(
        ...
        bet=BetSettings(
            ...
            percentage_gap=0,                  # Gap difference between outcomesA and outcomesB (for SMART strategy)
            max_points=0,                   # If the x percentage of your channel points is gt bet_max_points set this value
            stealth_mode=True,                  # If the calculated amount of channel points is GT the highest bet, place the highest value minus 1-2 points Issue #33
            delay_mode=DelayMode.FROM_END,      # When placing a bet, we will wait until `delay` seconds before the end of the timer
            delay=0,
            minimum_points=0,               # Place the bet only if we have at least 20k points. Issue #113
